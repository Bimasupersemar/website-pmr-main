...
<?php 
if(isset($_SESSION['pendaftaran_email']) == ''){ //belum dalam keadaan login
...
if(isset($_POST['simpan'])){
    ...
    if($password != ''){ //jika akan melakukan perubahan password
        $sql1 = "select * from pendaftaran where email = '".$_SESSION['pendaftaran_email']."'";
    ...
    if(empty($err)){
        $sql1 = "update pendaftaran set nama_lengkap = '".$nama_lengkap."' where email = '".$_SESSION['pendaftaran_email']."'";
        mysqli_query($koneksi,$sql1);
        $_SESSION['pendaftaran_nama_lengkap'] = $nama_lengkap;

        if($password){
            $sql2 = "update pendaftaran set password = md5($password) where email = '".$_SESSION['pendaftaran_email']."'";
...
<form action="" method="POST">
    <table>
        <tr>
            ...
            <td>
                <?php echo $_SESSION['pendaftaran_email']?>
            </td>
        </tr>
        <tr>
            <td class="label">Nama Lengkap</td>
            <td>
                <input type="text" name="nama_lengkap" class="input" value="<?php echo $_SESSION['pendaftaran_nama_lengkap']?>"/>
            </td>
        </tr>
        <tr>
